...
def get_thread_preview(thread_id, word_limit=30):
    """Return first user message truncated to `word_limit` words."""
    messages = load_conversation(thread_id)
    if not messages:
        return f"Empty chat ({thread_id})"
    ...
    words = first_user_msg.content.split()
